 ...
 * scheduler (wraps route re-renders in document.startViewTransition), and the
 * stream registry (live-loader stream delivery and reconnection).
 *
 * The session-channel store is deliberately absent: it installs itself from
 * `defineSessionChannel`, so an application that declares no channel never
 * pulls it into the always-loaded graph (#400).
 *
