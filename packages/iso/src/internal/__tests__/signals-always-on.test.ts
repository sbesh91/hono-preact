...
describe('signals are the always-on data layer', () => {
  ...
  it('@preact/signals enters the graph ONLY through the factory modules (core stays signals-free)', () => {
    ...
    // through `createStoreSignal`, and the reactive read hooks
    // (`useActionResult`, `useFormStatus`) project through its `useStoreValue`,
    // so neither the store modules nor the consumer hooks import
    // `@preact/signals` directly. The importer set stays confined to the factory
    // modules plus the barrel re-export.
