...
//   (`loaderUse`/`actionUse`) used by the route map builder.
//
...
// `ArrayExpression` literals (so a typo like `loaderUse = singleMw` fails
// the build instead of silently disabling the middleware at runtime).
