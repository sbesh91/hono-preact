...
describe('serverLoaderValidationPlugin', () => {
  ...
  it('fails when a *.server.* file has neither serverLoaders nor serverActions', () => {
    ...
    const { error } = transform(code, 'movies.server.ts');
    expect(error).toContain(
      "must export at least one of 'serverLoaders', 'serverActions', or 'serverSockets'"
    );
  });
  ...
  it('rejects a *.server.* file with pageUse as a named export (no longer recognized)', () => {
    const code = [
      'export const pageUse = [];',
      'export const serverActions = {};',
    ].join('\n');
    const { error } = transform(code, 'movies.server.ts');
    expect(error).toContain('found: pageUse');
  });
