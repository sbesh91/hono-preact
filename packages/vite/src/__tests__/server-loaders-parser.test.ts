...
describe('use exports', () => {
  ...
  it('detects a top-level loaderUse named export', () => {
    const program = parseProgram(`
      import { defineServerMiddleware } from '@hono-preact/iso';
      const mw = defineServerMiddleware(async (_c, next) => { await next(); });
      export const loaderUse = [mw];
      export const serverLoaders = { x: defineLoader(async () => ({})) };
    `);
    expect(hasNamedUseExport(program, 'loaderUse')).toBe(true);
  });

  it('returns false when loaderUse is absent', () => {
    const program = parseProgram(`
      export const serverLoaders = { x: defineLoader(async () => ({})) };
    `);
    expect(hasNamedUseExport(program, 'loaderUse')).toBe(false);
  });

  it('parseServerLoaders ignores a sibling loaderUse export', () => {
    const program = parseProgram(`
...
describe('findUseExports', () => {
  it('returns one entry per recognized use-export, with the init expression captured', () => {
    ...
    expect(found.map((e) => e.name).sort()).toEqual(['actionUse', 'loaderUse']);
  ...
  it('ignores a pageUse export (no longer a recognized use-export name)', () => {
    const program = parseProgram(`
      export const pageUse = [];
      export const loaderUse = [a];
      export const serverLoaders = {};
    `);
    const found = findUseExports(program);
    expect(found.map((e) => e.name)).not.toContain('pageUse');
    expect(found.map((e) => e.name)).toContain('loaderUse');
  });
