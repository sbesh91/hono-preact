...
describe('composeServerChain', () => {
  it('composes [app, page, unit] server middleware in outer->inner order', async () => {
    // The app tier is typed all-scope (`AppConfig['use']` is `AppUseElement`,
    // which admits only `ServerMiddleware<Scope>`) because it is dispatched in
    // all three; the page and unit tiers here carry the action scope under test.
    const app = defineServerMiddleware(async (_c, n) => {
      await n();
    });
  ...
  it("names the unit layer, indexed within the unit's own use", async () => {
    const ok = defineServerMiddleware(async (_c, n) => {
      await n();
    });
    await expect(
      composeServerChain<'action'>({
  ...
  it('keeps [app, page, unit] order when every layer is partitioned separately', async () => {
    const appMw = defineServerMiddleware(async (_c, n) => {
      await n();
    });
