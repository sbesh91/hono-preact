...
const UPDATE = (state, url) => {
	...
	if (url && url.type === "click") {
		...
		push = true;
		url.preventDefault();
		url = link.href.replace(globalThis.location.origin, "");
...
export function LocationProvider(props) {
	...
	const [url, route] = useReducer(
		UPDATE,
		props.url || globalThis.location.pathname + globalThis.location.search
	);
...
export function Router(props) {
	...
	toChildArray(props.children).some((/** @type {VNode<any>} */ vnode) => {
		const matches = exec(
			rest,
			vnode.props.path,
			(matchProps = { ...vnode.props, path: rest, query, params, rest: "" })
		);
