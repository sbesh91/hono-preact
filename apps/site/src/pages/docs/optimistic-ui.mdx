...
const Movies = ({ loaderData }) => {
  const {
    result,
    value: movies,
  } = useOptimisticAction(serverActions.create, {
...
`result` is a stub-compatible value: pass it to `<Form action={result}>` for declarative form submission, or call `result.mutate(payload)` for programmatic invocation. Both paths participate in the optimistic queue.
...
const NotesForm = ({ defaultNotes }) => {
  const {
    result,
    value: notes,
  } = useOptimisticAction(serverActions.setNotes, {
    base: defaultNotes,
...
For the imperative path, the same `result` value exposes `result.mutate(payload)` and `result.pending` directly, so you can call it from an `onClick` handler or await it in an async function without holding a separate `useAction` ref.
