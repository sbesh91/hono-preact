...
`valueSignal` holds the same projection as `value`, as a `ReadonlySignal<TBase>` (see [Signals](/docs/signals)). Reach for it when the surrounding render is worth skipping. `value` is simpler, and fine everywhere else.
