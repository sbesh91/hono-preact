...
When the component that owns the action also renders something expensive, hand the list off to a child and give it `addMovie.signal` instead:
...
const Movies = ({ loaderData }) => {
  ...
  return (
    <>
      <MovieList movies={addMovie.signal} />
...
`signal` is a `ReadonlySignal<TBase>` holding the same projection as `value` (see [Signals](/docs/signals)). Reach for it when the surrounding render is worth skipping. `value` is simpler, and fine everywhere else.

### Options

| Option       | Type                             | Description                                                                                             |
| ------------ | -------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `base`       | `TBase`                          | The base value (typically loader data) the projection layers over. Pass a stable reference (see below). |
...
`base` must be a stable reference across renders (the real data itself). The projection tracks it reactively, so a value built fresh on every render, an inline `?? []` when the loader field is missing, an inline `.filter(...)`, or a spread, re-derives the projection every render and, because your component binds the returned value, re-renders in a loop. Give the loader field a stable empty default, or memoize the expression, before passing it.
...
`movies` is a `ReadonlySignal<TBase>`; read `.value` to get the current projection, or pass the signal to a leaf so only that leaf re-renders (see [Signals](/docs/signals)). `addOptimistic(payload)` appends a queue entry and returns an `OptimisticHandle`:
