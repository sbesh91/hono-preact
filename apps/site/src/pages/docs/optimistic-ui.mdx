...
`valueSignal` is a `ReadonlySignal<TBase>` holding the same projection as `value` (see [Signals](/docs/signals)). Reach for it when the surrounding render is worth skipping. `value` is simpler, and fine everywhere else.
