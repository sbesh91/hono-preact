...
Route changes trigger a view transition automatically in browsers that support `document.startViewTransition`. The framework's generated client entry wraps the `onRouteChange` callback fired by preact-iso's `<Router>` and dispatches `startViewTransition` for every successful navigation; you do not opt in. Style the transition via `::view-transition-old(root)` / `::view-transition-new(root)` CSS, and respect `prefers-reduced-motion` to disable it.
