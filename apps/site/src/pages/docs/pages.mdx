...
A leaf `view` registered without `definePage` silently loses its route error boundary. Dev builds warn about this, but only when that route's lazy `view` thunk actually resolves, which happens on the first visit to the route rather than at registration. A registered route that is never visited during a session never warns, so a green console is not proof every leaf view uses `definePage`; a route only clears this check once it has actually been navigated to.
...
`about.server.ts` sits next to `about.tsx`, so its `serverLoaders` and `serverActions` are wired into the generated `server.tsx` (`createServerEntry`) automatically. See [Colocated server modules](/docs/routes#colocated-server-modules).
...
```ts
{
  ...
  children: [
    // `movies-list.server.ts` / `movie.server.ts` siblings are auto-discovered.
    { path: '',    view: () => import('./pages/movies-list.js') },
    { path: ':id', view: () => import('./pages/movie.js') },
  ],
}
```
