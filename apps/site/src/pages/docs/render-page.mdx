...
| `options.appConfig`    | The default-exported result of `defineApp()`. Surfaces app-level config to the render: middleware composition via `use`, plus opt-in features like [`speculation`](/docs/link-prefetch). The framework's generated server entry threads this for you; custom server entries must pass it explicitly to enable those features. |
