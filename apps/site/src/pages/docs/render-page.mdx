...
`renderPage` is public (see [Usage](#usage) above), so a custom catch-all can call it directly when you need bespoke layout injection or a `defaultTitle`. The loader and page-action handlers are not a public hand-wiring surface, though: they need the route-resolver factories (`makePageUseResolver`, `makePageActionResolvers`) that live behind `hono-preact/server/internal/runtime`, a private, version-coupled contract the codegen emits. Those resolvers carry your page-level `use` guards (including auth gates), so the handlers require one and refuse to construct without it, rather than composing a chain that silently drops your guards. Hand-assembling a full entry therefore depends on that internal door: pin your framework version and expect it to change in any non-major release. Short of an alternate runtime, prefer `api.ts` plus the generated entry.
