...
Head tags (`<title>`, `<meta>`, `<link>`) are collected during prerender via hoofd. Pages set them with the `useTitle`, `useMeta`, and `useLink` hooks (see [Head Management](/docs/head)):
