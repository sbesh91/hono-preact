...
**`useFieldErrors(name)`** returns just that field's messages (`string[]`) instead of the whole map, and re-renders only when that field's own errors change: a sibling field's error appearing, changing, or clearing does not re-render this component. Prefer this form when a field's own error display lives in its own component, and the whole-map form when one component renders several fields' errors together.
...
function useFieldErrors(name: string): string[];
...
With a `name`, returns just that field's messages (`string[]`, `[]` when the field is valid) and subscribes only to that field: a change to a sibling field's errors does not re-render the caller. Without a `name`, returns the whole map and subscribes to every field, so any field's error change re-renders the caller.
