...
points at the client build output; `assets.binding` exposes those files to the
Worker as `env.ASSETS`, which the framework reads at render time to emit
`modulepreload` hints for the client entry's boot chunks. No `run_worker_first` list is needed: the
...
assets and SSR. The built server resolves its client build relative to its own
file location, not the process working directory, so it runs correctly under
a systemd unit with no `WorkingDirectory` set or a Docker image with a
different `WORKDIR`.
