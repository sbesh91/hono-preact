...
For a rest param (`:rest*`, `:rest+`), pass `string[]` to build real slash-separated segments: `buildPath('/docs/:slug+', { slug: ['a', 'b'] })` builds `/docs/a/b`, encoding each entry individually. A plain `string` is still accepted and is encoded whole, so embedded `/` characters become `%2F` rather than additional segments.
...
Pass your own `aria-current` to override the computed value, including `aria-current={undefined}` to opt out entirely (the link still gets its active class, just no `aria-current` attribute). Active state is path-only: it cannot see the query string, so if a set of links differ only by their query (e.g. `?tab=`), manage `aria-current` yourself.
...
### Prefetching a link's data

Pair `prefetch` with `prefetchLoaders` to warm a link's target route before the click, so the navigation that follows has no loading state at all:
...
- `'hover'`: warms on hover intent (a short debounce on `pointerenter`, cancelled if the pointer leaves before the delay elapses) and on `focus`, so keyboard navigation gets the same warming as a mouse.
- `'visible'`: warms once the link scrolls into the viewport, then stops watching. Use it for links a user is likely to click without ever hovering, such as a card in a list.

`prefetchLoaders` names the loaders to warm explicitly, either a single `LoaderRef` or an array of them; there is no auto-discovery from `href`. Both props are additive: they compose with `activeClass` / `inactiveClass`, `match`, and `exact` with no interaction between them. See [Link Prefetch](/docs/link-prefetch) for the underlying `usePrefetch` hook this wraps.
...
| `transition` | `boolean` | `true`  | Set `false` to navigate without a view transition.                      |

## API reference

`<NavLink>` forwards any other `<a>` attribute (e.g. `target`, `rel`, `data-*`) to the underlying anchor and sets `aria-current="page"` while the link is active. A caller-supplied `aria-current` always wins, and `aria-current={undefined}` suppresses the computed value. Because active detection is path-only, links that differ only by query string need their `aria-current` managed explicitly.
...
| `transition`      | `boolean`                               | `true`   | Set `false` to navigate without a view transition.                                        |
| `prefetch`        | `'hover' \| 'visible' \| false`         | `false`  | Trigger for warming `prefetchLoaders`. No trigger means no prefetching.                   |
| `prefetchLoaders` | `LoaderRef \| ReadonlyArray<LoaderRef>` | none     | Loaders to prefetch for this link's target route. Required for `prefetch` to do anything. |
