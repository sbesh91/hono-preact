...
A `reload()` that fails to reconnect does not cost you what is already on screen. Under `.Boundary` + `useData(initial, reduce)`, the retained chunks are kept until the new connection delivers its first chunk, so a reconnect that never arrives leaves the last good fold rendered with `status === 'error'`, and a successful one swaps in the new stream. Retrying is therefore safe to offer as a button: the worst case is that nothing changes.
