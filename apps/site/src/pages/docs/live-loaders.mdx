...
const ActivityBar = activityLoader.View<ActivityEvent[]>(
  ({ data, status }) =>
    status === 'connecting' ? (
      ...
      <Feed events={data} status={status} />
    ),
  {
    initial: [],
...
- `loader.View(render, { initial, reduce })` is the accumulating form of the standard consumption convention. It folds every chunk into accumulated state and exposes a `status` field. `status === 'connecting'` on SSR and before the first chunk arrives; after the first chunk `status` is `'open'`. Branch on `status` inside the render function to show a connecting affordance.
...
Passing `initial` and `reduce` selects the accumulating form of `.View`. `data` becomes the folded accumulator and the render args carry a `status`. Branch on `status === 'connecting'` to render the pre-first-chunk state.
