...
All other `defineLoader` options (`cacheKeyParams`, `cache`, `timeoutMs`, `use`) work the same for live loaders. See [Server Loaders](/docs/loaders) for the full option table.

### `loader.View(render, { initial, reduce })` (accumulating form)

Passing `initial` and `reduce` selects the accumulating form of `.View`. The render function receives a `StreamState<Acc>` discriminated union as its first argument and the component's own props as its second; switch on `status` (`connecting | open | closed | error`). The `connecting` arm carries no data; the other arms carry the folded accumulator as `data`.
