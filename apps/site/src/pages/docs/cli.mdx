...
> **Using npm?** npm intercepts bare flags (`npm warn Unknown cli config "--adapter"`) and would otherwise scaffold the default adapter. Put framework flags after a `--` separator: `npm create hono-preact my-app -- --adapter=node`. pnpm, yarn, and bun forward bare flags directly. If you do forget the `--`, the scaffolder recovers the flag from npm's config, but npm still prints its warning.
