...
api.ts (your Hono app)   <- composes first
  POST /__loaders        <- framework RPC
  POST /__actions        <- framework RPC
  GET *                  <- framework SSR
```

That ordering is the whole story. A `.use('*', ...)` in `api.ts` runs on every request the framework will see, including the SSR page render and both RPC endpoints. A `.use('/__actions', ...)` runs on RPC actions. A `.get('/healthz', ...)` works the same way it does in any Hono app.

## Reserved paths

The framework owns three URL surfaces:

| Path              | Purpose           |
| ----------------- | ----------------- |
| `POST /__loaders` | Server loader RPC |
| `POST /__actions` | Server action RPC |
...
Scope to `/__actions` if you only want CORS on the RPC surface, or to your custom API routes if your SSR pages are same-origin only.

### CSRF protection

For cookie-authenticated `multipart/form-data` actions, see the dedicated [CSRF Protection](/docs/csrf) page. It explains when you need it, why the JSON path through `/__actions` is already protected by browser CORS preflight, and how to write an Origin check that covers the multipart gap.
