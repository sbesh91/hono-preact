...
Link prefetch makes navigation feel instant by fetching destinations before the user clicks. Enable it once with `speculation: true` in your app config; the framework emits a Speculation Rules tag that tells supporting browsers to prefetch same-origin links on hover. The feature is off by default; browsers that don't support Speculation Rules ignore the tag silently.
