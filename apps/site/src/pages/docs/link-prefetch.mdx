...
`<NavLink>` wraps this same hook behind a `prefetch` prop, so a nav link that
should warm its target's loaders on hover or on scrolling into view needs no
manual event binding at all. See [Active Links](/docs/active-links#prefetching-a-links-data)
for that form.

## See also

- [Active Links](/docs/active-links): `<NavLink>`'s built-in `prefetch` / `prefetchLoaders` props.
