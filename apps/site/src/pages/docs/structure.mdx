...
- **`serverLoaderValidationPlugin`** fails the build if a `.server.*` file has named exports other than `serverLoaders`, `serverActions`, or the recognized `use` exports (`loaderUse`, `actionUse`).
