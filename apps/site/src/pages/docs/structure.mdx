...
export default defineRoutes([
  ...
  { path: '/demo/projects/:projectId', view: () => import('./pages/project.js') },
...
- **Page bindings**: `definePage(Component, { errorFallback, Wrapper })` factory plus `<Page>`, `WrapperProps`. Used by views that need a Wrapper. Page-layer middleware is declared via `use` on the route node in `routes.ts`; entries are built with `defineServerMiddleware`, `defineClientMiddleware`, and `defineStreamObserver`. Loader consumption (`.View()`) lives inside the page's JSX tree; loading UI is rendered by switching on the `LoaderState` status inside `.View()`.
- **Loaders**: `defineLoader`, `LoaderRef`, `useReload`. (`LoaderRef` carries `useData()`, `useError()`, `invalidate()`, `cache`, `.View()`, and `.Boundary`. `useData()` returns a `ReadonlySignal`.)
- **Actions**: `defineAction`, `useAction`, `useOptimistic`, `useOptimisticAction`, `<Form>`.
- **Signals**: `@preact/signals` primitives (`signal`, `computed`, `effect`, `batch`, `untracked`, `useSignal`, `useComputed`, `useSignalEffect`, and the `Signal`/`ReadonlySignal` types) are re-exported directly from `hono-preact`. See [Signals](/docs/signals).
- **Caching**: `createCache` (advanced: shared caches across loaders).
- **Middleware**: `defineServerMiddleware`, `defineClientMiddleware`, `defineStreamObserver`, `defineApp`, `defineSessionChannel`, plus the outcome constructors `redirect`, `deny` (and `render` at the `hono-preact/page` subpath) and predicates `isOutcome` / `isRedirect` / `isDeny` / `isRender`. See [Middleware](/docs/middleware).
...
The framework's generated server entry imports the default export from `src/app-config.ts` (configurable via the `appConfig` option to `honoPreact()`). If the file doesn't exist, the framework treats it as `defineApp({})`. Creating or deleting `src/app-config.ts` while the dev server is running restarts it automatically, so the change takes effect without a manual restart. The `appConfig.use` array is the outermost layer of the middleware chain: it wraps every loader, action, and page render. See [Middleware: The three layers](/docs/middleware#the-three-layers).
...
- **`serverLoaderValidationPlugin`** fails the build if a `.server.*` file has named exports other than `serverLoaders`, `serverActions`, `serverRooms`, or `serverSockets`.
