...
`useParams` requires the named route to be the active one; when it is not, the object it returns does not have the shape it projects, and dev builds warn. Reach for [`useRouteMatch`](/docs/active-links#useroutematch) when the route may legitimately not be active: it returns the match or `null` instead of projecting a shape that is not there.
...
The list view consumes its loader data via `loader.useData().value`; the layout never sees it. If a child needs data the layout ALSO needs to display, lift the data into the layout's render via context or pass it through as a prop in the empty-path child.
