...
The check is **scoped to routes with a live guard**: it throws at boot only when at least one of the three tiers that could read the mismatched param is non-empty (app-level `use` from `defineApp`, the route-node `use` chain, or the room's own `use`). When all three are empty, there is no guard anywhere that could misread the missing param, so the boot is allowed to proceed; a one-time dev-only console advisory still names the mismatch, so it is never silent even though it doesn't block the boot. When congruence holds (names line up correctly), a similar dev-only advisory names each correspondence (which route param is satisfied by which channel key), so the pairing is never silent even when the names happen to match. Either way, the underlying reason is the same: the room's guard chain reads its params from the resolved **channel** key, not the route, so a route param the channel doesn't guarantee would leave a guard reading it as `undefined`.
...
A route param outside the supported `:name` class (one or more of `[A-Za-z0-9_]`, optionally followed by a single `?`/`*`/`+`) is **invisible to this check**, and to room param resolution generally: for example a hyphenated `/board/:board-id`. preact-iso's own runtime route matcher still binds that segment fine over plain HTTP, but its value never reaches `pathParams` for a realtime connection, so a room guard on such a route must not rely on that param.

This has different consequences depending on how the room reaches that route. A **colocated** room (no `serverRoute` binding) is not rejected: it simply never sees the non-conforming param, as described above. An **explicitly bound** room (`serverRoute(r).room`, or an equivalent `serverRoute(r).socket`) on such a route fails the boot outright: `serverRoute('/board/:board-id').room(...)` throws at startup rather than running with an empty params object, so every `/__loaders` request, action POST, and `/__sockets` upgrade in the app returns a 500 until the route segment is renamed to only use letters, digits, and underscores.
