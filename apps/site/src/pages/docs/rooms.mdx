...
3. **Join from the client** with `serverRooms.<name>.useRoom({ key })` or the free `useRoom(ref, opts)` form. The hook returns a `members` roster signal, a `send` function, and `setPresence` for updating this client's state.
...
export default function Board() {
  ...
  function handleMouseMove(e: MouseEvent) {
    ...
    setPresence({ ...pos, name: self.value?.state?.name ?? 'Me' });
...
`members`, `memberIds`, `member(id)` and `self` are all signals: read `.value` and that component subscribes. `members.value` updates on every join, leave, or presence change. `self.value` is this client's own roster entry, `undefined` until the first server snapshot arrives. Incoming messages from other members go to `onMessage`; they do not trigger a re-render.

Reading `.value` during render is the common case, and it is what subscribes your component. To drive something imperatively instead (a canvas layer, a chart, any non-Preact widget), subscribe rather than reading in an effect:

```tsx
// Runs on every roster change, for as long as the component is mounted.
useEffect(() => members.subscribe((roster) => drawAvatars(roster)), []);
```

A plain `useEffect` that _reads_ `members.value` in its body sees the roster once and never again: an effect body is not a tracking context, so nothing subscribes. Use `.subscribe()` (above) or `useSignalEffect`, both of which do.

## Per-member updates

`members.value` gives you the whole roster, and reading it re-renders your component whenever anything about anyone changes. For a handful of cursors that is exactly what you want. For a roster of fifty, one person moving their mouse re-renders all fifty rows.

`memberIds` and `member(id)` split that apart. `memberIds` changes only when someone joins or leaves. `member(id)` changes only when that one member's presence changes. Bind a row to one member and only that row re-renders:

```tsx
import type { ReadonlySignal } from 'hono-preact';
import type { PresenceMember } from 'hono-preact';
import { serverRooms } from './board.server.js';
import type { CursorState } from './board.server.js';

type CursorBinding = ReadonlySignal<PresenceMember<CursorState> | undefined>;

function Cursor({ member }: { member: CursorBinding }) {
  // Reading `.value` here subscribes THIS component. When someone else moves,
  // this one does not re-render.
...
export default function Board() {
  ...
  // `memberIds.value` changes on join and leave only, so this list re-renders
  // when membership changes, not when someone moves.
  return (
    <>
      {memberIds.value.map((id) => (
        <Cursor key={id} member={member(id)} />
      ))}
    </>
  );
}
...
The binding stays valid across joins, presence updates, leaves, and reconnects: an id that has not joined yet yields a binding that goes live when it does, and a member who leaves takes their binding to `undefined` rather than leaving it frozen on their last position. So a row can be rendered before its member arrives, and survives them leaving.

You get the same signal back every time you ask for a given id, so passing the binding down as a prop and calling `member(id)` again on a later render behave identically. Pick whichever reads better.

Both are worth it when the roster is large or presence updates are frequent. Below that, `members.value` is simpler and there is nothing wrong with it.

[Signals](/docs/signals) covers the reactive value these return, including the two mistakes that do not produce a compile error.
...
- An **explicitly bound** room (`serverRoute(r).room`, or the equivalent `serverRoute(r).socket`) **fails the boot closed** on a mismatch, but only when a guard could actually read the mismatched param, that is, when at least one of the three tiers is non-empty: app-level `use` from `defineApp`, the route-node `use` chain, or the room's own `use`. With all three empty, no guard can read the param, so the boot proceeds and prints a console advisory naming the mismatch instead. Either way the mismatch is never silent.
- A **colocated** room (no `serverRoute` binding) **never fails the boot** for a mismatch. Its route is its module's mount, and it carries no param wire, so its guard chain reads the missing param as `undefined`. It prints the same console advisory instead, whether a guard reads that param today or you add one later. That advisory (like the socket one below) prints in production too, not just in dev: it is your only signal that the mismatch is live.
...
- **Plain sockets (`defineSocket`/`useSocket`) run in a Durable Object.** On Cloudflare, the worker guards the upgrade at the edge and forwards it to a per-connection Durable Object running under the Hibernation API; on Node they run in-process. See [WebSockets](/docs/websockets) for setup details.
...
| `members`     | `ReadonlySignal<ReadonlyArray<PresenceMember<State>>>`       | The presence roster. Read `.value`; changes on any join, leave or presence update.                                                                                                           |
| `memberIds`   | `ReadonlySignal<readonly string[]>`                          | Membership ids. Changes on join and leave only. Read `.value`.                                                                                                                               |
| `member`      | `(id) => ReadonlySignal<PresenceMember<State> \| undefined>` | One member's entry. Changes only when THAT member changes, so a row bound to it re-renders alone. `undefined` when the id is not in the room. See [Per-member updates](#per-member-updates). |
| `self`        | `ReadonlySignal<PresenceMember<State> \| undefined>`         | This client's own roster entry. Read `.value`; `undefined` until the first snapshot arrives.                                                                                                 |
...
| `closeInfo`   | `SocketCloseInfo \| undefined`                               | The last close event's `code`, `reason`, and `wasClean`.                                                                                                                                     |
