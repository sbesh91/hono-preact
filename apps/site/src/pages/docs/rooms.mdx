...
Hold the binding `member(id)` gives you. It stays valid across joins, presence updates, leaves, and reconnects: an id that has not joined yet yields a binding that goes live when it does, and a member who leaves takes their binding to `undefined` rather than leaving it frozen on their last position. Passing it down as a prop, as above, is the point.

Calling `member(id)` again on every render works too, but it throws away the subscription each time and you lose the granularity you came for. Reach for it once, per row.
