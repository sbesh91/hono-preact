...
Every params map a room sees — `onJoin`'s `ctx.params` and the guard's `ctx.location.pathParams` — is a **null-prototype object**: read it by destructuring, `params.roomId`, `'roomId' in params`, `Object.keys(params)`, or `Object.hasOwn(params, 'roomId')`. A missing key reads as `undefined` for _any_ key name, including one that collides with an `Object.prototype` member like `constructor` or `toString`, which is what makes a guard like `if (!params.roomId) return deny()` safe even when a channel names a key after one of those. Don't call `params.hasOwnProperty(...)` or `params.toString()` directly on the object; it has no inherited methods to call. Use `Object.hasOwn(params, k)` instead.
