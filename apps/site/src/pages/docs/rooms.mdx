...
`members` gives you the whole roster, and reading it re-renders your component whenever anything about anyone changes. For a handful of cursors that is exactly what you want. For a roster of fifty, one person moving their mouse re-renders all fifty rows.
...
Both are worth it when the roster is large or presence updates are frequent. Below that, `members` is simpler and there is nothing wrong with it.
