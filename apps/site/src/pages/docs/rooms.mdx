...
- An **explicitly bound** room (`serverRoute(r).room`, or the equivalent `serverRoute(r).socket`) opted into the route contract, so a mismatch is a bug in that binding. It **fails the boot closed**, but only when a guard could actually read the mismatched param, that is, when at least one of the three tiers is non-empty: app-level `use` from `defineApp`, the route-node `use` chain, or the room's own `use`. If all three are empty, no guard could misread the param today, so the boot proceeds and prints a console advisory naming the mismatch instead. Either way the gap is never silent.
- A **colocated** room (no `serverRoute` binding) **never fails the boot** for a mismatch, whether or not a guard tier is live. Its effective route is just its module's mount, an accident of file layout rather than a contract you authored, and the framework has no param wire to give it either way. Failing the boot here would break an app the moment it added a guard anywhere in the chain, so the room prints the same console advisory instead: the guard chain reads the missing param as `undefined`, whether a guard reads it now or you add one later. That advisory (like the socket one below) prints in production too, not just in dev. It is the only signal you get that the mismatch is live.
