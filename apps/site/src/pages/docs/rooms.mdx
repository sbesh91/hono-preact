...
The binding name must be exactly `HONO_PREACT_REALTIME` (the adapter reads `c.env.HONO_PREACT_REALTIME`). The class name must be exactly `HonoPreactRealtimeDO` (the name the generated entry re-exports).
