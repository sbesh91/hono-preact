...
```tsx
const MovieCard = dataLoader.View<{ highlight: boolean }>((s) => {
  if (s.status === 'loading') return <MovieSkeleton />;
  return (
    <article class={s.highlight ? 'highlight' : undefined}>
      <h2>{s.data.movie.title}</h2>
    </article>
  );
});
...
const Summary = summary.View((s) => {
  const { reload } = useReload();
  if (s.status === 'loading') return <SummarySkeleton />;
  if (s.status === 'error') return <ErrorBox onRetry={reload} />;
  return <SummaryCard data={s.data} />;
});
...
const Similar = similar.View((s) =>
  s.status === 'loading' ? (
    <SimilarSkeleton />
  ) : (
    <SimilarCarousel items={s.data} />
  )
);
...
function HeaderWithSummary() {
  const s = summary.useData();
  return (
    <header>
      <BreadcrumbTrail />
      {s.status === 'loading' ? <Skeleton /> : <h1>{s.data.title}</h1>}
      <ActionsBar />
    </header>
  );
}
...
const Feed = serverLoaders.activity.View((s) =>
  s.status === 'loading' ? null : <ActivitySidebar events={s.data} />
);
...
const MovieView = movieLoader.View((s) =>
  s.status === 'loading' ? (
    <MovieSkeleton />
  ) : (
    <article>{s.data.movie.title}</article>
  )
);
