...
`@preact/signals` primitives (`signal`, `computed`, `useComputed`, `effect`, and the rest) are available directly from `hono-preact`, so a component that reads loader signals does not need a separate dependency.
