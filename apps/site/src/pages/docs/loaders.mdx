...
`match(state, handlers)` is the recommended way to consume that union inside a render function: it dispatches on `status` and hands each handler its fully narrowed member, so a handler for `success` reads `data` with no guard while a handler for `loading` cannot read `data` at all. A missing arm is a compile error:

```tsx
import { match } from 'hono-preact';

const CountView = statsLoader.View((state) =>
  match(state, {
    loading: () => <p>Loading...</p>,
    success: (s) => <p>{s.data.count} items</p>,
    revalidating: (s) => <p>{s.data.count} items</p>,
    error: (s) => <p>Failed to load.</p>,
  })
);
```

This is what makes a legitimately falsy value safe: a count of `0` rides the `success` arm and renders as `0 items`, never as the loading placeholder, because presence is carried by `status` rather than by testing `data` for truthiness. A direct `switch (state.status)` reads the same union and stays fully supported; `match` is a narrower, exhaustive-by-default way to write the same dispatch.

**Prop passthrough** (using the generic `P`): the render function's second parameter is whatever props the component is mounted with. Nothing about that shape can collide with the loader state, since the two arrive as separate parameters:
...
### Reading a denied loader

On the client-navigation path a `deny()` from a loader or one of its guards
reaches the boundary as a `LoaderDenyError`, carrying the whole envelope rather
than only a message:

```tsx
import { LoaderDenyError } from 'hono-preact';

function errorFallback({ error }) {
  if (error instanceof LoaderDenyError) {
    if (error.code === 'UNAUTHORIZED') return <SignInPrompt />;
    return <Denied status={error.status} message={error.message} />;
  }
  return <GenericError />;
}
```
...
| --------- | ----------------------- | --------------------------------------------------------- |
| `status`  | `number`                | The HTTP status the deny carried. Authoritative.          |
| `code`    | `DenyCode \| undefined` | The typed reason, when the deny declared one.             |
| `data`    | `unknown`               | Whatever the guard attached.                              |
| `message` | `string`                | The deny message (`Error.message`).                       |
| `name`    | `string`                | Always `'LoaderDenyError'`, for `instanceof`-free checks. |

A deny whose `data` carries validation issues raises the more specific
[`LoaderValidationError`](/docs/validation) instead, so schema failures keep
...
`loader.useData()` gives descendants the loader's data as a `ReadonlySignal`, without prop-drilling. Call it inside a `.View()` render function or inside an explicit `loader.Boundary`, and read `.value` to get the current state:

```tsx
function HeaderWithSummary() {
  const { status, data } = serverLoaders.summary.useData().value;
  // Narrow on `status`, the discriminant. Testing `data` for presence looks
  // equivalent and is not: a loader that legitimately resolves to `0`, `''` or
  // `null` would render the skeleton forever on a settled `success`.
...
Because `useData()` returns a signal, a binding that reads only `.value` inside a small leaf (or a `useComputed`) re-renders on its own, independent of the surrounding tree. Project a field with `useComputed`:

```tsx
import { useComputed } from 'hono-preact';

function Title() {
  const state = serverLoaders.summary.useData();
  const title = useComputed(() => state.value.data?.title ?? 'Loading...');
  return <h1>{title}</h1>;
}
```

`useData` is available on streaming loaders too, in a different shape: `loader.useData(initial, reduce)` folds each chunk through `reduce` (the same reducer shape the accumulating `.View(render, { initial, reduce })` form takes) and returns a `ReadonlySignal<StreamState<Acc>>`. `loader.Boundary` on a streaming loader runs in collect-mode: it provides the raw chunk stream on context, and every `useData(initial, reduce)` call underneath folds that same stream independently:

```tsx
function ActivityTotal() {
  const total = serverLoaders.activity.useData(0, (acc) => acc + 1);
  return (
    <p>{total.value.status === 'connecting' ? '...' : total.value.data}</p>
  );
}

function ActivityPanel() {
  return (
    <serverLoaders.activity.Boundary>
      <ActivityTotal />
    </serverLoaders.activity.Boundary>
  );
}
...
`@preact/signals` primitives (`signal`, `computed`, `useComputed`, `effect`, and the rest) are available directly from `hono-preact`, so a component that reads loader signals does not need a separate dependency. See [Signals](/docs/signals) for what they are and where the sharp edges are.
...
### `ctx.c` is an untyped Context

`ctx.c` is Hono's `Context` with its default environment, which means `c.env` is untyped. Every binding read typechecks, including one that does not exist: `c.env.MY_KV` and `c.env.MY_KVV` are equally acceptable to the compiler, and the mistake surfaces at runtime as a read of `undefined`. The route table has `RegisteredRoutes` to make `pathParams` and `buildPath` typed against your real routes; there is no counterpart for bindings, so nothing carries your `wrangler.jsonc` or `.env` shape into `ctx.c`. Typing bindings this way is planned API surface, not something you can register today.

Until then, declare the shape once and read bindings through it, so a typo is a compile error at every call site rather than at none:
...
// `c.env` is untyped, so this narrows rather than widens; keep it as the only
// place the app reads bindings off a raw Context.
export const bindings = (c: Context): AppBindings => c.env;
```
...
export const serverLoaders = {
  default: defineLoader(async ({ c }) => {
    const movies = await getMovies(bindings(c).MY_KV);
    return { movies };
  }),
};
```

Keeping one accessor also gives you a single place to update when a binding is added or renamed, and a single seam to stub in tests.
...
### Typing a loader written out of line

A function written inline in `defineLoader(...)` or `route.loader(...)` needs no annotation: the constructor types its context for you. When the function lives on its own, as a named `const` or a helper two loaders share, `LoaderFn` is that type:

```ts
import { defineLoader, type LoaderFn } from 'hono-preact';

const listMovies: LoaderFn<{ movies: MovieList }> = async ({ c, signal }) =>
  getMovies({ token: getCookie(c, 'session'), signal });

export const serverLoaders = {
  default: defineLoader(listMovies),
};
```

`LoaderFn<T>` is the route-independent single-value form, and its generics mirror the `LoaderRef<T>` the constructor hands back. The second one is the streaming discriminant: `LoaderFn<T, true>` is an `async function*` yielding `T`. The third supplies path params for a function that reads `ctx.location`, as in `LoaderFn<Movie, false, RouteParams<'/movies/:id'>>`; pass it only when you need it, since a route-independent `LoaderFn` asks less of its context and so composes with `route.loader` as well.

## Search-param dependencies (`cacheKeyParams`)

By default, a loader's cache key includes only the path and path params. Declare search-param dependencies per loader with the `cacheKeyParams` option:
...
The RPC request always sends the full location to the server; `cacheKeyParams` only narrows the client-side cache key.

## Cross-page invalidation

When a mutation on one page should refresh data on another, import the other page's loader ref and pass it to `useAction({ invalidate: { clear: [...] } })`. Invalidation is by reference, not by name:
...
const { mutate } = useAction(serverActions.addReview, {
  invalidate: { clear: [moviesLoaders.default] },
});
```

Pass `{ refetchActive: true }` to invalidate the current page's loader only. Use `loader.invalidate()` to clear the cache imperatively.

## Caching navigation results

Every loader gets its own cache automatically. It holds the most recent result, and it is read when a loader mounts: leaving a route and coming back to the same cache key skips the RPC call.

Two limits are worth knowing, because both are easy to assume away. The cache holds **one entry per loader**, so visiting a second key evicts the first. And it is read only on a loader's **first render**, so changing params without leaving the route (`/movies/1` to `/movies/2` under `/movies/:id`) refetches every time, including back to a key you just visited. Neither is a deliberate policy; see [#360](https://github.com/sbesh91/hono-preact/issues/360).
...
| ---------------- | ------------------ | ------- | --------------------------------------------------------------------------- |
| `cacheKeyParams` | `string[] \| '*'`  | `[]`    | Search params that change the cache key; `'*'` means any. Route-bound only. |
