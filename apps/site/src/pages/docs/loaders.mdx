...
`location.pathParams` and `location.searchParams` are ordinary objects: read them however you like, by destructuring, `pathParams.id`, `'id' in pathParams`, `Object.keys(pathParams)`, `Object.hasOwn(pathParams, 'id')`, or `pathParams.hasOwnProperty(...)`. A guard like `if (!pathParams.id) return deny()` is safe from the prototype-chain read hazard (an absent key named after an `Object.prototype` member like `constructor` resolving the inherited member instead of `undefined`) because a route cannot **declare** a param named after one of those: `serverRoute('/plugin/:constructor')` throws at definition. With the name rejected at the source, no param a route provides ever collides with an inherited member. The same contract applies to a socket's `data` factory params and a room's `onJoin` params; see [WebSockets](/docs/websockets) and [Rooms](/docs/rooms).
