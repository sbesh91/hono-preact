...
**`src/routes.ts`** wires the URL to the view; the colocated `movie.server.ts` sibling is auto-discovered, so no `server:` field is needed ([details](/docs/routes#colocated-server-modules)):
