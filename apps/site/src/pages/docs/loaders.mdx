...
const MoviesView = dataLoader.View(({ data, loading }) => {
...
On the client, loaders are state-based: the render function receives `{ data, loading, error, reload }` directly on every render rather than suspending. `data` is `undefined` on a cold load and retains the previous value during a reload (stale-while-revalidate). SSR still awaits each loader before rendering, so the initial HTML arrives with data present.
...
```ts
loader.View<P = {}>(
  render: (args: P & { data: Serialize<T> | undefined; loading: boolean; error: Error | null; reload: () => void }) => ComponentChildren
): FunctionComponent<P>
```

The render function receives `{ data, loading, error, reload }` plus any props declared by the generic `P`. `data` is `undefined` on a cold load before the first response; during a reload it retains the previous value. Branch on `loading` and `error` to render all states yourself. Inside the render function's subtree, `loader.useData()` also works for descendants that need data without prop-drilling.
...
The `error` argument is non-null on a fetch error. Use `reload` to let the user retry:

```tsx
const StatsView = statsLoader.View(({ data, loading, error, reload }) => {
...
const Summary = summary.View(({ data, loading, error, reload }) => {
  if (loading) return <SummarySkeleton />;
  if (error) return <ErrorBox onRetry={reload} />;
  return <SummaryCard data={data!} />;
});

const Cast = cast.View(({ data, loading }) =>
  loading ? <CastSkeleton /> : <CastList items={data!} />
);

const Similar = similar.View(({ data, loading }) =>
  loading ? <SimilarSkeleton /> : <SimilarCarousel items={data!} />
);
...
function HeaderWithSummary() {
  const { data, loading } = summary.useData();
  return (
    <header>
      <BreadcrumbTrail />
      {loading ? <Skeleton /> : <h1>{data!.title}</h1>}
      <ActionsBar />
    </header>
  );
}
...
const Feed = serverLoaders.activity.View(({ data, loading }) =>
  loading ? null : <ActivitySidebar events={data!} />
);
...
const MovieView = movieLoader.View(({ data, loading }) =>
  loading ? <MovieSkeleton /> : <article>{data!.movie.title}</article>
);
