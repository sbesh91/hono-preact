...
the component filters the rendered options against it and handles navigation,
ARIA wiring, and selection commit. It ships unstyled: style it
...
**Filtering is built in.** Each `<Combobox.Option>` matches its label against
the typed query with the `filter` prop, which defaults to the exported
`matchSubstring` helper; non-matching options unmount. Pass a custom
`(label, query) => boolean` to change the match, or `filter={null}` to disable
filtering entirely, the escape hatch when you filter server-side (or in an
effect) and render only the matching options yourself. Options with non-string
children need `itemToString` so the filter has a label to match. `create`
options always stay visible.
...
(and `onInputChange`) is always the prefix the user typed, so filtering is
identical across all three autocomplete modes.

## Demo

The minimal form is a single `Combobox.Input`. Focus it (or click it) to open,
type to filter; the component narrows the options and handles navigation and
selection. A single select commits and closes on pick.
...
chips with `Combobox.Value`, which exposes `{ selectedItems, remove }` to its
`render` prop. It renders a `<span>` and accepts standard HTML attributes (like
`class`):
...
turns off auto-highlight; pair it with `filter={null}` for a static,
non-filtering suggestion list.

**Async options.** Set `filter={null}`, fetch and filter in an effect, and
render the results; override `Combobox.Status` with a render prop to announce
loading instead of the result count.
...
`Combobox.Value` exposes `{ selectedItems, remove }` via its `render` prop.
...
| `filter`            | `((label: string, query: string) => boolean) \| null`                        | `matchSubstring` | Match an option's label against the typed query; failing options unmount. `null` disables filtering (server-side filtering escape hatch).          |
...
| Prop       | Type                                                 | Default | Description                                                                                   |
| ---------- | ---------------------------------------------------- | ------- | --------------------------------------------------------------------------------------------- |
| `children` | `ComponentChildren`                                  | -       | Static content; use `render` to display the selected items.                                   |
...
The default filter, `matchSubstring(label: string, query: string): boolean`, a
case-insensitive substring match, is also exported from `hono-preact-ui` for
composing your own filter around it.
