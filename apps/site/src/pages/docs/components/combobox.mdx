...
Multiple selection keeps the popup open and renders the picks as removable chips
through `Combobox.Value`. Backspace on an empty input removes the last chip.
...
The combobox is uncontrolled by default. Pass `value` / `onValueChange`,
...
stays open on pick. Render the selected chips with `Combobox.Value`, which
exposes `{ selectedItems, remove }`:
...
```tsx
<div className="inline-flex min-w-64 items-center rounded-lg border border-zinc-200 bg-white focus-within:border-zinc-900 dark:border-zinc-800 dark:bg-zinc-900 dark:focus-within:border-zinc-50">
  <Combobox.Input className="min-w-0 flex-1 bg-transparent px-3 py-2 text-sm text-zinc-900 outline-none placeholder:text-zinc-400 dark:text-zinc-100" />
  <Combobox.Trigger className="px-2 text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-300" />
</div>
