...
`Select.Value` a render prop and resolve the label from your own data, which
...
>
  <Select.Trigger>
    <Select.Value
      ...
      render={(props, { selectedLabels }) => (
        <span {...props}>{selectedLabels.join(', ') || 'Assign to'}</span>
      )}
    />
  </Select.Trigger>
  {/* ... */}
...
placeholder unless you supply a render function. Default element `<span>`.
...
| `children`    | `ComponentChildren`                        | -       | Static content overriding the default label display. |
