...
```css
/* Center the modal and give it a card surface. position/translate restore
   centering even when a reset (e.g. Tailwind preflight) clears the UA margin
   that normally centers a modal <dialog>. `translate` is used for centering so
   `transform` stays free for the entry animation. */
...
section below) and forwards unknown props to the element it renders.
`Dialog.Trigger`, `Dialog.Popup`, and `Dialog.Close` also expose
`data-state="open" | "closed"` and pass `{ open }` to a render function.
...
| Function   | `render={(props, state) => <a {...props}>{state.open ? 'Close' : 'Open'}</a>}` | Full control; `state` is `{ open }` for Trigger, Popup, and Close.                |
...
| ---------------------- | -------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| `useRender`            | `useRender(opts) => VNode`                                                       | Resolves a `render` prop (element, string, or function) to a VNode, merging framework-controlled props onto it. |
| `RenderProp`           | `RenderProp<State>`                                                              | The `render` prop type: `VNode \| string \| ((props, state) => VNode)`.                                         |
