...
| Page  | `use` on a route node in `src/routes.ts`                       | The matched node and its descendants: renders, actions, and route-bound loaders. A bare `defineLoader` RPC is route-independent (see below). |
...
A `use` on a route node inherits down the tree: it applies to the node's own view (if any) and every descendant. It covers the render path for the whole subtree, the action RPC path, and the RPC path of any **route-bound** loader (`serverRoute(r).loader`). You declare the guard once on the ancestor; leaves under it are gated without repeating anything. A sibling route that is not a descendant of the guarded node stays ungated.

Bare loaders are the exception. A route-independent `defineLoader` runs its SSR render inside the route chain, but its client-navigation RPC (`POST /__loaders`) runs only the app-level `use` and the loader's own unit-level `use`, never the route node's page-layer `use`. So a bare loader colocated under a guarded route is gated on first paint but not on a direct RPC hit. Bind the loader with `serverRoute(r).loader` (as in the example above) or give it a unit-level `use` to gate it everywhere. Actions have no such gap: a bare `defineAction` inherits the route's guard by matching its POST URL, though `serverRoute(r).action` is still recommended under an authenticated layout (see [Server Actions](/docs/actions#route-bound-actions)).
