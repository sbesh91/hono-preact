...
The same primitive replaces the old `defineServerGuard` / `defineClientGuard` / `defineActionGuard` family. Page bindings, loaders, and actions all accept the same `use` array, and the framework partitions server- vs client-bound members by their `runs` tag.
