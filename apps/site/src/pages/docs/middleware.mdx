...
The context's shape follows the scope: page and loader scope carry a `location` (path and search params), while action scope does not (`ServerActionCtx` is `{ c, signal, module, action, payload }`). A guard that reads `ctx.location.pathParams`, such as a per-resource gate declared on a route node, therefore applies to renders and loaders only; enforce the same rule for actions inside the action body or its `input` schema, which receive the validated payload.
