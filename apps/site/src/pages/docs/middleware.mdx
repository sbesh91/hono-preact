...
- **`pageUse` on `.server.*`** is consumed by `loadersHandler` and `actionsHandler` directly, around every `POST /__loaders` and `POST /__actions` request that belongs to this page. The page React tree never renders for those calls, so the page-tsx `use` binding wouldn't fire.
