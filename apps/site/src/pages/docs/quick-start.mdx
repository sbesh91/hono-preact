...
The route entry in `src/routes.ts` needs no `server` field: `movies.server.ts` sits next to `movies.js`, so the build auto-discovers the sibling and wires it in.
