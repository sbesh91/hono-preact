...
The framework also ships a `nodeAdapter()` from `hono-preact/adapter-node` for non-Cloudflare deployments; see [Build & Deploy](./deployment) for the full picture. See [Vite Config](/docs/vite-config) for all `honoPreact()` options.
...
Register the route in `src/routes.ts`; the colocated `movies.server.ts` is wired in with it:
...
`defineLoader(fn)` returns a typed `LoaderRef<T>`. The Vite `moduleKeyPlugin` rewrites the call at build time to inject `{ __moduleKey, __loaderName }` so the key drives RPC routing (`/__loaders`), the `__id` Symbol identity, and HMR. `.View(render)` returns a Preact component pre-wrapped in the loader's error boundary, data context, and reload context. The render function receives a `LoaderState` discriminated union as its first argument and the component's own props as its second, on every render; switch on `status` (`loading | success | revalidating | error`) to render each state, and read `reload` from `useReload()`. `loader.useData()`, called inside a `.View()` render function or a `loader.Boundary`, is argument-free and returns a `ReadonlySignal<LoaderState>`; read `.value` to get the current state.
...
The route entry in `routes.ts` doesn't change; actions ride along with the same `server` import. Submit a title in the form. The action runs on the server and the list updates. To automatically re-fetch the loader after the action, pass `invalidate: { refetchActive: true }` via `useAction` (see [Server Actions](/docs/actions) for the programmatic form).
...
- [Middleware](/docs/middleware): protect pages with the unified `use` array.
- [Loading States](/docs/loading-states): switch on the `LoaderState` status to show a spinner or skeleton while the loader fetches.
