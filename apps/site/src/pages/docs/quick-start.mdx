...
The scaffold runs `pnpm install` for you. Pass `--adapter=node` if you're targeting Node.js instead of Cloudflare Workers (with npm, put framework flags after `--`: `npm create hono-preact my-app -- --adapter=node`); see [Build & Deploy](./deployment) for the deployment side.
