...
Like loaders, an action can be bound to its route with `serverRoute(r)`. Calling `.action(fn)` is otherwise identical to `defineAction` (an action reads its data from the payload, so there is no typed `location`), and it accepts the same options (`use`, `timeoutMs`, `input`). It types the payload the same three ways too (see [Typing the payload](#typing-the-payload)) — the example below infers it from an `input` schema, so no `<Payload, Result>` generics are needed:
...
Binding the route makes the action resolve its page-level `use` chain from that exact route pattern. A bare `defineAction` resolves the chain by matching the POST URL, which is correct whenever the patterns are unambiguous. Reach for `route.action` when an action lives under an authenticated layout: the route-level gate (a `requireSession` on the parent route node, say) is then applied by the action's own pattern, closing the window where two overlapping patterns (`/a/:x` and `/a/:y`) match the same URL and the wrong route's gates run. The same `route` binder typically already defines the page's loaders, so its loaders and actions read as one route-bound unit.
...
`useAction` posts to the page URL with `Accept: application/json` and returns a discriminated result. The action body runs identically whether called from `<Form>` or `useAction`. `useAction` also accepts an opt-in `schema` option for client-side pre-validation; see [Validation](/docs/validation#pre-validating-an-imperative-action).
...
For route-wide gating (every action, plus every route-bound loader, under a route), declare `use` on the route node in `src/routes.ts`. See [Middleware](/docs/middleware) for the full chain model, including why a bare `defineLoader` RPC is route-independent. To bind an action to that route node's gate by exact pattern, define it with [`route.action`](#route-bound-actions).
