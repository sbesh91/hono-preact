...
## Where an action can be invoked from

The POST goes to the current page URL, and the handler resolves `(module, action)` in two steps: the URL's route contributes its own `serverActions` plus its ancestors', and a miss there falls back to a module-key lookup over the [`src/server` registry](/docs/routes#the-srcserver-registry). So an action declared in a route's own `.server.ts` is addressable only from that route's URL, which is what keeps its route `use` chain in force on every invocation.

An action a shared shell or layout fires from many pages therefore belongs in the `src/server` registry, where it is module-key addressable from any page (gate it with a unit-level `use`, since it runs no route node's page-layer `use`). Invoking a route's own action from a different page answers `404` with `Action '<name>' not found on '<path>'`.
...
| `invalidate` | `InvalidateInput`           | Same semantics as `useAction`'s `invalidate`: `{ refetchActive: true }` re-runs the active loader; `{ clear: [...] }` clears each ref (and re-runs the active loader if it appears in `clear`). |
...
| ------------ | ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `invalidate` | `InvalidateInput`         | `{ refetchActive: true }` re-runs the current page's `serverLoader` (via the `/__loaders` RPC in the browser). `{ clear: [...] }` invalidates each [`LoaderRef`](/docs/loaders)'s cache; pass loaders imported from other pages to refresh data across the app. Omit to do nothing. |
...
When a mutation on one page should refresh data on another, import the other page's loader and pass it to `invalidate.clear`. Invalidation is by reference; there are no cache names:
...
const { mutate } = useAction(serverActions.addMovie, {
  invalidate: { clear: [ratingsLoader] }, // clears ratingsLoader's cache on success
});
```

`clear` accepts an array of `LoaderRef`s, so a single action can refresh multiple loaders in one shot: `invalidate: { clear: [moviesLoader, ratingsLoader] }`.

Use `refetchActive: true` (instead of `clear`) to invalidate the current page's own loader after the action succeeds.
