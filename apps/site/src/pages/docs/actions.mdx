...
`useActionResult(stub?)` returns a `ReadonlySignal` of the result of the most recent action invocation that targeted the current page render. Read `.value` to get the current `ActionResult`. The `submittedPayload` field lets you re-populate form inputs via `defaultValue` so users don't lose what they typed on a deny.
