...
`deny` is a `DenyRecord`: `status`, `message`, an optional `code` from the
[`DenyCode`](/docs/actions#deny-codes) vocabulary, and the optional `data`
the guard attached.
...
with no `use` array. This applies just as much to a guard registered at the
route or app level (an entry composed into the chain above the action) as to
one listed directly in the action's own `use` array: what determines whether
`TDenyData` is inferred is which spelling of `defineServerMiddleware` wrote
the guard, not where in the chain it runs. A route- or app-level guard
written with the argument form unions in cleanly; one written with the type
argument form contributes `unknown` at every call site that uses it.
