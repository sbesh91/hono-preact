...
The `navigated` arm resolves the promise instead of leaving it pending forever: when the action redirects to a same-origin destination, the caller's `await mutate(payload)` settles with `{ ok: true, kind: 'navigated' }`. `onSuccess` does not fire for this arm (there is no result to pass it), but any loaders the action declares as invalidated still run, and `pending` clears. Because navigation may have already unmounted the component that called `mutate`, code after the `await` must not assume the component is still mounted.
