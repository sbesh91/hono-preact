...
| --------- | -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `mutate`  | `(payload) => Promise<MutateResult<TResult, TDenyData>>` | Fires the action. Resolves with a four-arm discriminated union, uniformly discriminated by `kind`, so awaiting callers can chain on the result without leaking unhandled rejections: `{ ok: true; kind: 'success'; data }`, `{ ok: true; kind: 'navigated' }`, `{ ok: false; kind: 'deny'; deny }`, or `{ ok: false; kind: 'error'; error }`. The `navigated` arm settles when the action redirects to a same-origin destination: `onSuccess` does not fire for it, declared loaders are still invalidated, and `pending` is cleared, but there is no `data` to read. Both failure arms also write the hook's `error` state and call `onError`, for non-awaiting render-time use. Stable reference, safe to pass to `useEffect` or memoized children. |
...
#### Handling all four arms

`mutate`'s result is a four-arm discriminated union, uniformly discriminated by `kind`. Switch on `kind` exhaustively rather than checking `ok` alone, so a redirect, a denied request, and a thrown error each get distinct handling:
...
**Unmount aborts the request.** If a component that called `mutate` unmounts while the request is in flight, the fetch is aborted. No error outcome is recorded, so nothing surfaces through `useActionResult`. `onError` does still fire, with an abort error: an optimistic wrapper needs it to revert its pending entry rather than strand it as permanently in-flight. Guard on `err.name === 'AbortError'` if your handler should skip cancellations.
