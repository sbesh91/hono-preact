...
The same policy applies to middleware-chain composition: when an action cannot compose its chain -- because the page-use resolver failed, or because a `use` entry (app-level, page, or the action's own) is not a middleware or a stream observer -- the action never runs and the 500 message carries the underlying detail only in dev.
