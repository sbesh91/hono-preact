...
async function submit(payload) {
  ...
  switch (result.kind) {
    case 'success':
      ...
      break;
    case 'navigated':
      // The action redirected to a same-origin destination. Declared
      // loaders were already invalidated; there is no `data` to read.
      break;
    case 'deny':
