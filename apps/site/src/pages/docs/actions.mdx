...
`onMutate` and `onError` bracket a dispatched request. A server `deny`, error, or timeout rejects the in-flight mutation, so `onError` fires (with the `onMutate` snapshot for rollback). A client-side `schema` rejection is different: it short-circuits **before** any request, so neither `onMutate` nor `onError` fires. The failure is still surfaced every other way, as the returned `{ ok: false, error }`, the hook's `error` state, and a `deny(422)` on `useActionResult` (decode the issues with `getValidationIssues`), so it renders identically to a server-caught validation failure. See [Validation](/docs/validation#pre-validating-an-imperative-action).
