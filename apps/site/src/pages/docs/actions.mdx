...
`routeServerModules` walks the route manifest and produces every `server` deferred import in the record shape `actionsHandler` consumes. The handler routes by `mod.__moduleKey` — the path-derived key injected into each `.server.*` file by `moduleKeyPlugin`.

`actionsHandler` also accepts a raw `import.meta.glob` result if you want to wire it directly; the `routeServerModules` adapter just builds the same shape from the route manifest.
...
import { useAction } from 'hono-preact';
import { loader as ratingsLoader } from './ratings.server.js';
import { serverActions } from './movies.server.js';
...
// Method form:
const { mutate, pending } = serverActions.addMovie.useAction({ invalidate: [moviesLoader] });

// Equivalent function form:
const { mutate, pending } = useAction(serverActions.addMovie, { invalidate: [moviesLoader] });
```
