...
| --------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `mutate`  | `(payload) => Promise<MutateResult<TResult, TDenyData>>` | Fires the action. Resolves with a three-arm discriminated union so awaiting callers can chain on the result without leaking unhandled rejections: `{ ok: true; data }`, `{ ok: false; kind: 'deny'; deny }`, or `{ ok: false; kind: 'error'; error }`. Both failure arms also write the hook's `error` state and call `onError`, for non-awaiting render-time use. Stable reference, safe to pass to `useEffect` or memoized children. |
...
#### Handling all three arms

`mutate`'s result is a three-arm discriminated union. Switch on it exhaustively rather than checking `ok` alone, so a denied request and a thrown error get distinct handling:
