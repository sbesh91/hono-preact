...
`/__actions`), the framework's reserved paths are mounted ahead of user
middleware in the generated entry, so a Hono `csrf()` middleware on
`/__actions` is not user-configurable.
