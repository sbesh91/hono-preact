...
If you need a stricter posture, mount Hono's `csrf()` middleware on
`/__actions` from your `src/api.ts` user app. The user app is mounted
ahead of the framework's reserved RPC paths, so user middleware runs
before the action dispatcher. See [CSRF Protection](/docs/csrf) for the
full recipe.
