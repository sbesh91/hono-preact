...
A **colocated** socket, a `.server.ts` sibling of a route with no `serverRoute` binding, has no param wire: `ctx.location.pathParams` stays empty and the connection is never denied for a missing param, even beside a route with `:param`s. When the route declares params and at least one guard tier (app-level `use`, the route's page-layer `use`, or the socket's own `use`) is live, a one-time dev-only console advisory names the socket and the params a guard on it would read as `undefined`, so the gap is never silent even though the boot proceeds. Reach for an explicit `serverRoute(r).socket` binding whenever a socket needs to authorize on a route param. Rooms resolve their params differently, from the channel key rather than this `params` option; see [Rooms](/docs/rooms).

A route param outside the supported `:name` class (one or more of `[A-Za-z0-9_]`, optionally followed by a single `?`/`*`/`+`), for example a hyphenated `/board/:board-id`, is invisible to the params wire above: preact-iso's own runtime route matcher still binds that segment fine over plain HTTP, but its value never reaches `ctx.location.pathParams` for a realtime connection. A colocated socket on such a route is not rejected; it simply never sees that param, the same as any other route param a colocated socket doesn't wire up. An **explicitly bound** `serverRoute(r).socket` on such a route fails the boot outright rather than running with an empty params object: `serverRoute('/board/:board-id').socket(...)` throws at startup, so every `/__loaders` request, action POST, and `/__sockets` upgrade in the app returns a 500 until the route segment is renamed to only use letters, digits, and underscores.
