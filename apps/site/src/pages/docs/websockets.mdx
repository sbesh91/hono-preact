...
Import the `serverSockets` map from the `.server` module and pass the entry to `useSocket`:

```tsx
// src/chat.tsx
import { useSocket } from 'hono-preact';
import { serverSockets } from './chat.server.js';

export default function Chat() {
  const { send, status, lastMessage } = useSocket(serverSockets.chat, {
    lastMessage: true,
    onMessage(msg) {
