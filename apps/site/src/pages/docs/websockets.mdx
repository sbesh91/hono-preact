...
A **colocated** socket, a `.server.ts` sibling of a route with no `serverRoute` binding, has no param wire: `ctx.location.pathParams` stays empty and the connection is never denied for a missing param, even beside a route with `:param`s. It never fails the boot for this either, regardless of the route: the framework has no param wire to give a colocated socket, so failing the boot would break an app the moment it added any guard anywhere in the chain. When the route declares a param a guard could read and at least one guard tier (app-level `use`, the route's page-layer `use`, or the socket's own `use`) is live, a console advisory names the socket and the params a guard on it would read as `undefined`, so the gap is never silent. This advisory is not dev-only: it prints in production too, since it is the only signal an author gets that the mismatch is live. Reach for an explicit `serverRoute(r).socket` binding whenever a socket needs to authorize on a route param. Rooms resolve their params differently, from the channel key rather than this `params` option; see [Rooms](/docs/rooms).

A route param outside the supported `:name` class (one or more of `[A-Za-z0-9_]`, optionally followed by a single `?`/`*`/`+`), for example a hyphenated `/board/:board-id`, is invisible to the params **wire** above: preact-iso's own runtime route matcher still binds that segment fine over plain HTTP, but its value never reaches `ctx.location.pathParams` for a realtime connection. It is not invisible to the colocated-socket **advisory**, though: that check reasons over every param a guard could actually read off the route (preact-iso's own matcher, not this framework's narrower `:param` grammar), so a hyphenated segment still fires the advisory above rather than staying silent. A colocated socket on such a route is not rejected; it just never sees that param, the same as any other route param a colocated socket doesn't wire up. An **explicitly bound** `serverRoute(r).socket` on such a route fails the boot outright rather than running with an empty params object, independent of guard liveness: `serverRoute('/board/:board-id').socket(...)` throws at startup, so every `/__loaders` request, action POST, and `/__sockets` upgrade in the app returns a 500 until the route segment is renamed to only use letters, digits, and underscores.

Every params map a socket sees (the `data` factory's second argument and the guard's `ctx.location.pathParams`) is an ordinary object: read it however you like, by destructuring, `params.id`, `'id' in params`, `Object.keys(params)`, `Object.hasOwn(params, 'id')`, or `params.hasOwnProperty(...)`. A guard like `if (!params.id) return deny()` is safe from the prototype-chain read hazard (an absent key named after an `Object.prototype` member like `constructor` resolving the inherited member instead of `undefined`) because a route cannot **declare** a param named after one of those: `serverRoute('/plugin/:constructor')` throws at definition. With the name rejected at the source, no param a socket sees ever collides with an inherited member.
