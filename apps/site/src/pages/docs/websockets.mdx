...
Every params map a socket sees — the `data` factory's second argument and the guard's `ctx.location.pathParams` — is a **null-prototype object**: read it by destructuring, `params.id`, `'id' in params`, `Object.keys(params)`, or `Object.hasOwn(params, 'id')`. A missing key reads as `undefined` for _any_ key name, including one that collides with an `Object.prototype` member like `constructor` or `toString`, which is what makes a guard like `if (!params.id) return deny()` safe even when a route names a param after one of those. Don't call `params.hasOwnProperty(...)` or `params.toString()` directly on the object; it has no inherited methods to call. Use `Object.hasOwn(params, k)` instead.
