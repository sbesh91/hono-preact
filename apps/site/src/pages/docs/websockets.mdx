...
`useSocket` manages the WebSocket lifecycle: it connects on mount, reconnects with exponential backoff on unexpected drops, and closes cleanly on unmount. `send` queues messages if the socket is not yet open, flushing them once the connection is established (up to 128 queued messages); beyond that cap, further sends while not open are dropped. The drop is silent in production and logs a console warning in development, and `send` returns nothing either way, so an app that must not lose messages while disconnected should watch `status` and hold them itself rather than rely on the queue. Toggling `enabled` to `false` on an open socket tears the connection down and `status` becomes `'closed'`.
