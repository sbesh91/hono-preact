...
Use `.View()` to create a component that re-renders on each new chunk. The render function receives `{ data, loading, error, reload }`: `loading` is `true` before the first chunk arrives, and `data` holds the latest yielded value after that.
...
const DashboardView = dataLoader.View(({ data, loading, error, reload }) => {
...
`loader.useData()` always returns the latest loader state. Components re-render on each new chunk automatically.
...
const Summary = summary.View(({ data, loading }) =>
  loading ? <SummarySkeleton /> : <SummaryCard data={data!} />
);
...
**Before the first chunk:** `loading` is `true` and `error` is null. If the stream errors before yielding any value, `loading` becomes `false` and `error` is set. The error boundary above catches render-time throws.

**After the first chunk:** `error` in the `.View()` render function surfaces the error; `data` keeps returning the last good value. The page stays mounted with stale data visible.
...
- [Loading States](/docs/loading-states): branching on `loading` while a stream fills.
