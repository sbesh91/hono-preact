...
Broader is not automatically better. App-level `use` publishes on every page in the app, which makes every document per-visitor and therefore uncacheable by a shared cache (see [Publishing makes the document per-user](#publishing-makes-the-document-per-user)). Scope the publish to the subtree that needs it.
