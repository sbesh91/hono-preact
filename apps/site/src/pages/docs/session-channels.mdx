...
type SessionChannel<T> = {
  ...
  publishToClient(ctx, value: T): void;
...
| Member            | Description                                                                                                                               |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `publishToClient` | Publish from a server middleware, passing its context. The value ships to the browser with the response. A no-op outside a request scope. |
...
const server = defineServerMiddleware(async (ctx, next) => {
  ...
  sessionChannel.publishToClient(ctx, { signedIn: Boolean(user) });
