...
The value is not persisted. It lives in memory for the life of the page, and each server round-trip replaces it wholesale. That is how a logout clears it: the logout action publishes `{ signedIn: false }` and the client leg starts denying. Clearing is always an explicit publish; a response that publishes nothing says nothing about the channel and leaves the stored value alone.
