...
**Publish from a scope at least as broad as every page a navigation can start from.** This is the placement rule that decides whether the guard works, and it is separate from where you enforce. A client leg reads the store as it stands when the navigation begins, before any loader RPC for the destination has been sent. Publish only on the guarded subtree and a signed-in visitor who cold-loads a page outside it and then navigates in reads `undefined` and is bounced to the login page, because nothing on that first response ever published. Put the publishing middleware in app-level `use` (see [Middleware](/docs/middleware)) and keep the redirect on the route node:

```ts
// src/app-config.ts
import { defineApp } from 'hono-preact';
import { publishSession } from './auth/require-session.js';

export default defineApp({ use: [publishSession] });
```
...
// App-level publisher: runs on every render and every RPC in the app, so a
// cold load of any page seeds the client store. Goes in app config's `use`.
...
`read()` returns `undefined` when no round-trip has published on this channel yet, and also when the two bundles disagree on the channel id (see [Where to declare a channel](#where-to-declare-a-channel)). That is **unknown**, not **unauthorized**. The channel says nothing about the visitor; it says nothing has spoken yet.

Your guard chooses how to read that, and the two readings trade different things:

- **Treat unknown as denied**, the way the example above does with `?.signedIn` (falsy for both the unpublished case and an explicit `{ signedIn: false }`). Then you must publish broadly enough that a signed-in visitor can never hit the unknown case, which means app-level publishing per the rule above. Publish too narrowly and the guard is a false bounce waiting to happen.
- **Let unknown pass and defer to the server guard.** The navigation proceeds, the loader RPC runs the authoritative server middleware, and an unauthenticated visitor is redirected a beat later. You trade the flash of a page that is about to be refused for never bouncing a visitor who is genuinely signed in.
