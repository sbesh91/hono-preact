...
Put **route-less** units here: a bare `defineLoader` (addressed by module key over the loaders RPC) or `defineAction` (addressed by module key too; it still runs behind the `use` gates of whichever page invokes it), plus rooms and sockets. A page reaches them by importing their `serverLoaders` / `serverActions` exactly as it would a route-attached module.

```
src/server/audit/log.server.ts   ->  export const serverLoaders = { recent: defineLoader(...) }
```

A unit bound to a specific route with `serverRoute('/path')` belongs next to that route, not here; a route-bound unit placed in `src/server` fails loudly at startup, because it has no route node to inherit its page gates from. The folder is configurable via the Vite plugin's `serverDir` option (default `src/server`).
