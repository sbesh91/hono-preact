...
**Route-less** units are addressed by module key: a bare `defineLoader` (over the loaders RPC) or `defineAction` (via the handler's moduleKey fallback; it runs behind the `use` gates of whichever page invokes it), plus rooms and sockets. A page reaches them by importing their `serverLoaders` / `serverActions` exactly as it would a route-attached module.
