...
| `use`      | `AppUseElement[]`                                        | optional                    | An array of middleware run for this node and all its descendants. Entries must handle **every** scope: a route node's `use` wraps the node's renders AND its route-bound loaders and actions, so write `defineServerMiddleware(...)` with no scope argument. A `Middleware[]` annotation no longer fits. See [Middleware](/docs/middleware#the-three-layers). |
