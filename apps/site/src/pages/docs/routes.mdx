...
| `use`      | `Middleware \| Middleware[]`                             | optional                    | Page-layer middleware attached to this node and all its descendants. See [Middleware](/docs/middleware#the-three-layers).                                                     |
...
`use` accepts a single middleware entry or an array. When multiple ancestors each carry `use`, they compose outer-to-inner in tree order (outermost ancestor first). A sibling that is not a descendant of the guarded node is not affected.
