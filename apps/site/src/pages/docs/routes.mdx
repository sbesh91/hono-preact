...
| `use`      | `RouteUseElement[]`                                      | optional                    | An array of middleware run for this node and all its descendants. Server entries must handle **every** scope: a route node's `use` wraps the node's renders AND its route-bound loaders and actions, so write `defineServerMiddleware(...)` with no scope argument. Client middleware (`defineClientMiddleware`) and stream observers are also accepted. See [Middleware](/docs/middleware#the-three-layers). |
