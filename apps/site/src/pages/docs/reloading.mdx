...
Reload is a background refresh: while `reloading` is `true`, `data` retains the previous value so the current content stays visible. Inside a `.View()` render function, `loading` is also `true` during a reload. Use `reloading` (from `useReload`) or `loading` (from `.View()`) to reflect the in-progress state in your UI.
...
- [Loading States](/docs/loading-states): branching on `loading` during a refetch.
