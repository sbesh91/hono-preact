...
const MoviesView = moviesLoader.View((s) => {
  ...
  // success | revalidating | error all carry `data`, narrowed to the value type.
  return (
    <ul>
...
Match the `loading` status and return a skeleton component:
...
const MoviesView = moviesLoader.View((s) => {
  if (s.status === 'loading') return <MoviesSkeleton />;
  return (
    <ul>
      {s.data.movies.results.map((m) => (
        <li key={m.id}>{m.title}</li>
...
| `status`         | Fields                               | When                                                                                      |
| ---------------- | ------------------------------------ | ----------------------------------------------------------------------------------------- |
| `'loading'`      | (none)                               | Cold load before the first response. No data yet.                                         |
...
In the `success`, `revalidating`, and `error` arms `data` is narrowed to `Serialize<T>`, so there is no `undefined` to guard. The render function also receives any props declared by the generic `P` (see [prop passthrough in loaders](/docs/loaders)). The imperative `reload()` callback is read from `useReload()`, not passed as a render arg (see [Reloading](/docs/reloading)).
