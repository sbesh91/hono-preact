...
`devSsrInclude` is checked before the built-in `/@` and `/node_modules/` prefix check. A string matches by prefix and a RegExp by `test()`, both against the request path with any query string removed. The built-in prefixes still apply to everything else.
