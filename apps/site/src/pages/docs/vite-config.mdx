...
| `serverDir`   | `string`                                                     | `'src/server'`                 | Registry folder for route-less server modules; globbed if it exists. See [The `src/server` registry](/docs/routes#the-src-server-registry). |
