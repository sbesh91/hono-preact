...
The route entry in `routes.ts` is unchanged; the `cache` rides along with the loader through the same `server` import:
...
Per-page concerns (loader, cache, Wrapper, fallback, errorFallback, serverGuards, clientGuards) live with the page component, not with the route declaration that mounts it. `definePage` captures them:
