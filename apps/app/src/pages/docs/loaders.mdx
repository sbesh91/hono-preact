...
**`serverLoaderValidationPlugin`** — fails the build if a `.server.*` file has named exports other than `serverGuards` or `serverActions`. If you write:
...
```
'.server files may only export 'serverGuards' or 'serverActions' as named exports (found: helper). Export the server loader as the default export only.'
```

`serverGuards` and `serverActions` are the only permitted named exports. Any other named export is an error.
