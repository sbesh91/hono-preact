...
The `Page` component (in `@hono-preact/iso`) runs the right loader depending on the environment.
...
**`serverOnlyPlugin`** — during the client bundle build, every import of a `*.server.*` file is rewritten to inert stubs. The default export becomes a no-op async function and `serverGuards` (if imported) becomes an empty array:
...
**`serverLoaderValidationPlugin`** — fails the build if a `.server.*` file has named exports other than `serverGuards`. If you write:
...
```
'.server files may only export 'serverGuards' as a named export (found: helper). Export the server loader as the default export only.'
```

`serverGuards` is the one permitted named export — use it to attach middleware-style guards that run before the loader on the server. Any other named export is an error.
