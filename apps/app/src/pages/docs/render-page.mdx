...
If a [route guard](/docs/guards) redirects during SSR, it throws a `GuardRedirect`. `renderPage` catches this and turns it into an HTTP redirect via `c.redirect()` — you don't need to handle it in the route handler.
