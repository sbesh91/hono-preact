...
The framework generates the Hono application as a virtual module (`virtual:hono-preact/server`) from your `routes.ts` and `server/layout.tsx`. It wires the `__loaders` and `__actions` handlers, the `location` middleware, and the catch-all SSR handler automatically. To add custom REST routes, author an optional `src/api.ts` that exports a Hono app (or a function returning one); the generated entry mounts it before the SSR catch-all.
...
Renders the full HTML shell: `<head>`, `<body>`, the `#app` mount point, and the `<script>` tag that loads the client bundle. The framework's generated server entry renders this component for the catch-all route and wraps its output in `<!doctype html>`.
