...
- **Page bindings**: `definePage(Component, { loader, cache, fallback, errorFallback, serverGuards, clientGuards, Wrapper })` factory plus `<Page>`, `WrapperProps`. Used by views that need loader/cache/Suspense bindings.
- **Loaders**: `defineLoader`, `LoaderRef`, `useLoaderData`, `useReload`.
- **Actions**: `defineAction`, `useAction`, `useOptimistic`, `useOptimisticAction`, `<Form>`.
- **Caching**: `createCache`, `cacheRegistry`.
