...
A layout is a Preact component whose props match `LayoutProps` from `@hono-preact/iso`:
...
A layout cannot declare `server`:

```ts
{
  path: '/movies',
  layout: () => import('./pages/movies-layout.js'),
  server: () => import('./movies.server.js'),  // ❌ throws at defineRoutes time
  children: [...],
}
```

This keeps the data model simple: the matched leaf is the only route with a loader, and it's the only loader that runs per request. If you want chrome plus data at the same URL, structure it as a layout with an empty-path child carrying the loader:
...
The list view consumes its loader data via `useLoaderData`; the layout never sees it. If a child needs data the layout ALSO needs to display, lift the data into the layout's render via context or pass it through as a prop in the empty-path child.
...
| Parallel parent + child loaders | Out of scope | Hoist the loader onto whichever route owns the data. |
| `useParentLoaderData` / loader composition | Out of scope | Same. |
| Path-prefix middleware beyond guards | Out of scope | Use Hono `.use()` in your `api.ts` for path-prefix middleware. |

These may land in later versions. v0.1 intentionally keeps the data model simple: one loader per leaf, no parallel data graph, no layout-vs-view loader coordination.
