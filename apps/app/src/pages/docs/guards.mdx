...
Guards live with the page component via `definePage` bindings, alongside the loader. If the same component needs different guards at different mount points, wrap it twice and route to each separately:

```tsx
export const AdminPage = definePage(Page, { serverGuards: ['admin'] });
export const SuperAdminPage = definePage(Page, { serverGuards: ['superadmin'] });
```

Then mount each export at its own path in `routes.ts`, pointing the `view` at a sub-export rather than the file's default. (The simplest pattern is one wrapped export per file.)
...
```ts
import { createGuard, defineLoader } from '@hono-preact/iso';
...
**`src/pages/admin.tsx`** is a pure component, with all bindings (loader and both guard chains) attached via `definePage`:

```tsx
import type { FunctionComponent } from 'preact';
import { definePage } from '@hono-preact/iso';
import { loader, serverGuards } from './admin.server.js';
import { clientGuards } from './admin.guards.js';
...
export default definePage(Admin, { loader, serverGuards, clientGuards });
```

**`src/routes.ts`** just wires the URL to the view + server module; no guard imports needed here:

```ts
{
  path: '/admin',
  view: () => import('./pages/admin.js'),
  server: () => import('./pages/admin.server.js'),
}
```
...
**`serverLoaderValidationPlugin`:** `.server.ts` files may only export `loader`, `serverGuards`, `serverActions`, and `actionGuards` as named exports (alongside the default loader). Any other named export fails the build:
