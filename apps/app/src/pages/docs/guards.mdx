...
Guards live with the page component via `definePage` bindings, alongside the loader and cache. If the same component needs different guards at different mount points, wrap it twice and route to each separately:
...
**`src/pages/admin.tsx`** is a pure component, with all bindings (loader, cache, and both guard chains) attached via `definePage`:

```tsx
import type { FunctionComponent } from 'preact';
import { definePage, useLoaderData } from '@hono-preact/iso';
import { loader, cache, serverGuards } from './admin.server.js';
...
export default definePage(Admin, { loader, cache, serverGuards, clientGuards });
```
