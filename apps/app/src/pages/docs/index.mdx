[← home](/)

# hono-preact

A full-stack template that pairs Hono's edge-ready HTTP layer with Preact's lightweight UI and preact-iso's SSR + hydration primitives, compiled by Vite and deployed to Cloudflare Workers.

## Stack

| Layer | Tool | Role |
|-------|------|------|
| HTTP server | Hono | API routes, middleware, catch-all SSR handler |
| UI | Preact + preact-iso | Components, client-side routing, SSR + hydration |
| Build | Vite | Dev server, two-pass production build |
| Styles | Tailwind CSS v4 | Utility-first CSS |
| Deploy | Cloudflare Workers | Edge runtime |

## Docs

- [Project Structure](/docs/structure) — what each file and folder does
- [Vite Configuration](/docs/vite-config) — configuring the build and dev server with `honoPreact()`
- [Rendering Pages](/docs/render-page) — SSR entry point, head tag injection, GuardRedirect handling
- [Adding Pages](/docs/pages) — routing conventions, lazy loading, MDX
- [Server Loaders](/docs/loaders) — isomorphic data fetching
- [Loading States](/docs/loading-states) — showing a fallback UI while a client loader fetches
- [Route Guards](/docs/guards) — protecting pages with middleware-style access control
- [Reloading Data](/docs/reloading) — imperatively re-running the client loader after a mutation
- [Server Actions](/docs/actions) — typed client-to-server mutations with `useAction` and `<Form>`, including file uploads and streaming
- [Action Guards](/docs/action-guards) — middleware-style checks that run before server actions
- [Build & Deploy](/docs/deployment) — Vite build pipeline and Wrangler
