...
3. `pnpm format:check`
4. `pnpm typecheck`
5. `pnpm typecheck:tests` (each package's `tsconfig.test.json`, fanned out by `pnpm -r`. It exists because every framework package's build `tsconfig` must keep excluding `src/**/__tests__/**` so tests stay out of the published `dist/`, which also means step 4 never typechecks a single test file; a second `noEmit` config is the only way to type them. `scripts/__tests__/typecheck-tests-gate.test.mjs` fails if a package with excluded tests lacks either the config or the script, since `pnpm -r --if-present` would otherwise skip it silently).
6. `pnpm test:types` (vitest typecheck mode; runs the `*.test-d.ts` type-level assertions that both of the configs above exclude).
7. `pnpm test:coverage` (or `pnpm test` if coverage isn't needed locally).
8. `pnpm test:integration`
9. `pnpm --filter site build`

If `format:check` fails, run `pnpm format` to fix and commit the result. Do not push commits that you have not personally seen pass these nine steps. The single biggest miss is `format:check`, which is fast to run and trivially fixable, but reliably forgotten.

Lighthouse runs in **CI only** and is deliberately **not** part of the nine steps above (it builds, serves the worker with `wrangler dev`, and drives Chrome, far too slow for a pre-push gate). The PR-only `lighthouse` job posts a soft sticky comment, reading its baseline from the `metrics` branch; the `main` push job pushes the `lighthouse-report.json` / `lighthouse-history.jsonl` / `lighthouse-badge.json` baselines to the dedicated orphan `metrics` branch (never to `main`), and the README badge reads `lighthouse-badge.json` from that branch. This keeps `main` history free of automated metric commits. Tooling: `@lhci/cli` (root devDependency); config in `.lighthouserc.json`; extraction and rendering in `scripts/measure-lighthouse.mjs` and `scripts/render-lighthouse-comment.mjs` (unit-tested under `scripts/__tests__/`). The LHCI flow needs two uploads: `temporary-public-storage` (hosted report links) and `filesystem` (writes the `.lighthouseci/manifest.json` the extractor parses). Client JS size (framework runtime per feature plus UI components) is tracked by the PR-only `client-size` job. It builds the framework on the PR head and on the base ref (in a `git worktree`), measures each with `scripts/measure-framework-size.mjs` (isolated esbuild bundles, peers external, gzip, marginal over core / ui-core), and posts a curated sticky comment via `scripts/render-framework-size-comment.mjs`. The same job also builds `apps/site` on both refs and measures the real always-loaded baseline (the entry chunk plus its static-import closure: what every route ships) via `scripts/measure-site-chunks.mjs`, rendered as a "Docs-site shipped JS" section, this is the shipped-reality number the isolated probe cannot show. There is no committed size baseline: the job diffs head versus base live, so nothing is committed on `main`.

**Page render smoke (`.github/workflows/smoke.yml`, `pnpm test:smoke`).** Boots real dev servers for both adapters and serves both apps' real built output, then fetches routes and asserts the status. It also walks the boot module graph of each target's rendered page (`smoke/module-graph.ts`) and asserts every module in it is served as JavaScript: a page can answer `200 text/html` with perfect SSR markup while the client it ships is dead, which is #392 (a module request answered with the SSR document, rejected by strict MIME checking, so nothing hydrated under `vite dev`). Dynamic imports are deliberately not followed, since a lazily-loaded route chunk is not part of booting. The checker is unit-tested in `smoke/__tests__/module-graph.test.ts`, because an opt-in suite's only assertion of a claim must not itself be unverified. It is **opt-in**: add the `run-smoke` label to a PR (it then re-runs on every subsequent push) or trigger it from the Actions tab. It is REQUIRED before merge; see the pre-merge gate in [`REVIEW.md`](REVIEW.md). It exists because v0.13.0 shipped four defects behind a fully green 3500-test suite (a production SSR 500, `Cycle detected` on every dev route under both adapters, and an example app that no longer built) -- all module-graph / bundling-order / build-pipeline faults, a class no unit test can catch because it cannot reproduce under Vitest's module resolution. Two harness details are load-bearing and must not be "tidied": the dev half passes `optimizeDeps: { force: true }` (the dup-instance bug reproduces only on a cold cache, and `pnpm dev` is `vite --force`), and the built half pins `NODE_ENV=production` (Vitest sets `NODE_ENV=test`, Vite derives `import.meta.env.PROD` from it, and the Node adapter's `serve()` boot lives inside `if (import.meta.env.PROD)` -- inheriting `test` silently strips the boot so the built entry exits 0 without listening). It lives in its own workflow because reacting to a label needs `labeled` on the `pull_request` trigger, which in `ci.yml` would re-run the whole matrix on every label change.

The `scaffold-smoke` job (ubuntu + windows) is also CI-only. It covers the scaffolder's own install + git-init path (`create-hono-preact` → `runChild` → `spawn`), which nothing else exercises: the unit suite stubs `spawn`, and `scaffold-integration` installs with `--no-install`. Windows is why it exists (#256): npm/pnpm are `.cmd` shims that `spawn` cannot launch without a shell. The non-obvious part is that it runs against a **local registry** (verdaccio, config in `.github/verdaccio.yaml`), into which it packs and publishes the workspace's own `hono-preact` / `hono-preact-ui`; everything else proxies to npmjs via an uplink. Two reasons, both load-bearing: resolving the template's `hono-preact` pin from the public registry would install the last *published* framework rather than the commit under test, and a release PR pins a version that does not exist yet (the lockstep policy bumps the template pin in the same commit as the version bump), which cannot resolve at all. Gotchas if you touch it: publish with `npm publish <tarball>`, never `pnpm publish`, whose "already published?" check ignores `--registry`, queries npmjs, and silently exits 0 having published nothing; the job needs its own build step because `pack` runs `prepack` while these packages build on `prepublishOnly`; and the "resolved from localhost" assertion is what stops a misconfigured registry from silently falling back to npmjs.
