...
**Goal:** Replace the synthetic `/live-stats` ticker with three coordinated streaming features on the movies surface: parallel multi-loader detail page (cast, similar movies, AI-style summary, box-office), layout-anchored live activity feed, and bucketed URL-driven streaming search.

**Architecture:** Four `defineLoader` async generators on `/movies/:id` wrapped in nested `<Loader>` components for parallel streaming sections. One `feedLoader` mounted in the `/movies/*` layout with a stable synthetic location so it survives soft navigation. `serverLoader` on `/movies` rewritten as a generator that yields bucketed search results when `?q=` is present.
