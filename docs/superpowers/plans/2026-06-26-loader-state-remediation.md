...
- [ ] **Step 1: `setError` helper** in `use-loader-runner.tsx`: `const setError = (err: unknown) => setPhase((p) => ({ tag: 'error', error: err instanceof Error ? err : new Error(String(err)), value: phaseValue(p) ?? syncDataRef.current }));` (the `?? syncDataRef.current` preserves the stale-while-error fix from commit 1f9578a, retaining a preloaded value on error; it is correct for the error phase only). Replace the 8 copy-pasted error-phase constructions (and the 4 inline `instanceof Error` normalizations) with `setError(err)`. Run the runner suite and confirm the preload stale-while-error test still passes.
