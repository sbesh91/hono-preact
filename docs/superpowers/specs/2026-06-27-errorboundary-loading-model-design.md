...
- **`RouteBoundary`** (`internal/route-boundary.tsx`, the framework's own `ErrorBoundary`) sits **above** the `Router` (the app shell composes `Page > Routes`) and does **not** set `this.__c`, so it is transparent to thrown promises. It only catches real errors and rethrows framework outcomes. It is unaffected by this change.
...
`SuspenseHost` returns `HostConsumer` directly instead of wrapping it in `PreactIsoErrorBoundary`. The promise `HostConsumer` throws now bubbles to the nearest `Router`, which holds `[cur, prev]` alive while the chain resolves. Outcomes (`render` / `deny` / `redirect`) are thrown as **non-promise** objects, so they continue to propagate past the `Router` (which only acts on promises) to the framework `RouteBoundary` / `renderPage` for translation. (Verified: the deny-propagation path is unchanged in a real Router tree.)
...
- **`<Page>` escape hatch (`internal.ts` documents hand-composing the pipeline)**: document the new contract that `PageMiddlewareHost` requires an ancestor suspense boundary (`Router`). The default `Page > Routes` pipeline already satisfies it; hand-composed pipelines must provide a `Router` or equivalent `__c` boundary.

## New contract (the one real cost)

`SuspenseHost` (hence `PageMiddlewareHost`) **now depends on an ancestor `__c` suspense boundary** (the `Router`) instead of being self-contained. In the real app this is always satisfied (`Page > Routes(Router)`). The only breakage is code that mounts `PageMiddlewareHost` without a `Router` ancestor: the 3 unit tests, and any future hand-composed pipeline. This is a deliberate, documented contract change, not an accident.
