...
`serverRoute(r).socket/.room` real binding itself is new in the unreleased base PR (#274) and stays non-breaking: nobody has shipped against it, `SocketRef`'s `Params` slot defaults to `{}` (every existing `SocketRef<In, Out>` reference and `defineSocket` call stays source-compatible), the `data` factory's second argument is optional to read (existing one-arg factories keep working), and the `SOCKET_ROOM_PARAM` -> `SOCKET_KEY_PARAM` rename is an internal contract constant, not a public export.

An earlier version of this note claimed the whole socket-side change set was non-breaking on that basis. That claim did not hold: several of the hardenings on this branch land on exports that WERE already released, and break code written against them. The full list:
