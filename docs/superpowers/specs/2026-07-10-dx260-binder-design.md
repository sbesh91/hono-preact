> DECISION (maintainer, 2026-07-10): Docs-only (option C). No new API. Teach serverRoute('<layout path>') as the layout-binding spelling, document the index-child aliasing (over-guarding direction) and the bind-a-descendant-leaf workaround for guard-only grouping prefixes, and migrate the site's projects-shell.server.ts onto bound loaders as the exemplar (which also silences the #263 dev warning for the demo).
