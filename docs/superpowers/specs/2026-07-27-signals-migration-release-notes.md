...
also exposes that signal directly, as `valueSignal`, so a caller that hands it
to a child gets the same leaf-level updates the primitive offers, and a caller
that never reads `value` is no longer subscribed at all.
