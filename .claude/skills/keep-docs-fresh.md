...
- Adding a new option or prop to any public API (`<Page>`, `defineLoader`, `useLoaderData`, `useAction`, `Form`, `defineServerGuard`, `defineClientGuard`, etc.)
...
2. **Grep the docs for it:**
   ```bash
   grep -r "changedSymbol" apps/site/src/pages/docs/
   ```
...
Every commit that changes public API behavior must stage **at least one doc file** alongside the code files. If you can't point to a doc file in `apps/site/src/pages/docs/` that was updated, the commit is not ready.
